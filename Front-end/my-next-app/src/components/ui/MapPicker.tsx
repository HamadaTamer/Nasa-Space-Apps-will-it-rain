...
const MAP_OPTIONS = {
    ...
    styles: [ // Minimalist style for a clean look
...
const MapPicker: React.FC<MapPickerProps> = ({ initialCoords, onSelectLocation }) => {
    ...
    if (loadError) {
        return <div className="p-6 text-center text-red-500">Error loading Google Maps. Check console and API key.</div>;
    }
    ...
    if (!isLoaded) {
        return (
            <div className="flex flex-col items-center justify-center h-[400px] bg-gray-200 dark:bg-gray-700 rounded-lg animate-pulse">
                ...
                <p className="text-gray-700 dark:text-gray-300">Loading Map Service...</p>
            </div>
        );
    }
